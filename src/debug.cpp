...
namespace jw::debug::detail
{
    ...
    [[nodiscard]]
    static char* encode(char* out, const void* in, std::size_t len)
    {
        ...
        auto* const ptr = static_cast<const std::uint8_t*>(in);

        for (std::size_t i = 0; i < len; ++i)
        {
            std::memcpy(out, hex + 2 * ptr[i], 2);
            out += 2;
        }
        return out;
    }
    ...
    template <typename T>
    [[nodiscard]]
    static char* encode(char* out, const T* in)
    {
        return encode(out, in, sizeof(T));
    }
